...
// 投稿者名が30文字以上かどうかをチェックする
if ($is_valid_auther_name && mb_strlen($input_author_name) > 30) {
    $is_valid_auther_name = false;
    $_SESSION['input_error_author_name'] = 'ニックネームは 30 文字以内で入力してください。
    （現在 ' . mb_strlen($input_author_name) . ' 文字）';
}
...
// 投稿内容が1000文字以上かどうかをチェックする
if ($is_valid_message && mb_strlen($input_message) > 1000) {
    $is_valid_message = false;
    $_SESSION['input_error_message'] = '投稿内容は 1000 文字以下で入力してください。
    （現在 ' . mb_strlen($input_message) . ' 文字）';
}

// ---------- 投稿をデータベースへ保存する処理 ----------
// 投稿者名、投稿内容ともに、バリデーションが完了しているなら、
// 投稿者名が空でもOK
if ($is_valid_auther_name && $is_valid_message) {
    ...
    // セッション変数の文字列をリセット
    $_SESSION['action_success_text'] = '投稿が完了しました。';
